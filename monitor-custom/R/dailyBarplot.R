...
# daily barplot plot using monitorPlot_latestDailyByHour() based on input
###########################

dailyBarplot <- function(dataList=NULL, infoList=NULL, textList=NULL) {
  
  logger.info("----- dailyBarplot() -----")
  ...
  monitorPlot_latestDailyBarplot(ws_monitor, monitorID=monitorID, lookbackDays = lookbackDays, size = style, minHours=18)
