...
if ( FALSE ) {
  ...
  # Must set working directory to monitor-custom/
  # Output is placed into monitor-custom/output/
  
  req <- list(
    path = "/monitor-custom/dev/dailyaveragetable",
    parameters = list(
      monitorids = "530330030_01,410510080_01,060750005_01",
      startdate = "20200910",
      enddate = "20201001"
    )
  )
  ...
  if ( TRUE) {
    ...
    setMonitorIDsScript <- "R/sharedUtils/setMonitorIDs.R"
    ...
    result <- try({
      ...
      source(setMonitorIDsScript)   # function to set monitor IDs
    }, silent = TRUE)
    stopOnError(result)
